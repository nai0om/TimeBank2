...
//###########################################################
	...
	public function action_user()
	{
        ...
        $user_obj = ORM::factory('user')->where('role','<>','1');
		$page = Arr::get($_GET, 'page');
		if ($page == '')
            $page = 1;
        $count = $user_obj->count_all();
		$total_page = ceil($count/20);
        $users = $user_obj->order_by('id','desc')
            ->limit(20)->offset(($page-1)*20)->find_all();
...
//###########################################################
	public function action_organization()
	{
		...
		if ($page == '')
            $page = 1;
        $count = DB::select()->from('organizations')->execute()->count();
		$total_page = ceil($count/20);
        $organizations = DB::select()->from('organizations')->from('organizations')->order_by('id','desc')
            ->limit(20)->offset(($page-1)*20)->execute()->as_array();
...
//###########################################################
	public function action_event()
	{
		...
		if (HTTP_Request::GET == $this->request->method())
        {
            $org = trim(Arr::get($_GET, 'org'));
            if( $org != '')
            {
                $events = $events->where('organization_id', '=', $org);
            }
		}
		$page = Arr::get($_GET, 'page');
		if ($page == '')
            $page = 1;
        $count = $events->count_all();
		$total_page = ceil($count/20);
        $events = $events->order_by('id','desc')
            ->limit(20)->offset(($page-1)*20)->find_all();
...
//###########################################################
	public function action_comment()
	{
        ...
        $comment_obj = ORM::factory('comment');
		$page = Arr::get($_GET, 'page');
		if ($page == '')
            $page = 1;
        $count = $comment_obj->count_all();
		$total_page = ceil($count/50);
        $comments = $comment_obj->order_by('id','desc')
            ->limit(50)->offset(($page-1)*50)->find_all();
